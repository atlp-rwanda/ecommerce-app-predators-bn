...
eventEmitter.on("wishlist:add", async product => {
  try {
    ...
    try {
      await _sendEmail.default.sendNotification(user.email, subject, text);
    } catch (error) {
...
eventEmitter.on("product:buy", async (product, buyer) => {
  ...
  await _sendEmail.default.sendNotification(seller.email, "Product bought", `The product "${product.name}" has been bought by ${buyer.name}`);
});
eventEmitter.on("product:update", async (product, oldPrice) => {
  ...
  if (product.price < oldPrice) {
    ...
    users.forEach(async user => {
      ...
      await sendNotification.sendSaleNotification(user.email, product.name, product.price, product.url);
    });
  }
});
...
eventEmitter.on("product:expiring_soon", async product => {
  ...
  Your Store Name
  `;
  await _sendEmail.default.sendNotification(seller.email, subject, expiring_soon);
});
eventEmitter.on("product:expired", async product => {
  ...
  Your Store Name
  `;
  await _sendEmail.default.sendNotification(seller.email, subject, expired);
});
eventEmitter.on("product:sold", async (product, quantity, totalSaleAmount) => {
  ...
  await _sendEmail.default.sendNotification(seller.email, subject, text);
  console.log("Notification sent to seller:", seller.name);
});
...
eventEmitter.on("product:updated", async product => {
  ...
  await _sendEmail.default.sendNotification(updatedUser.email, subject, text);
});
eventEmitter.on("product:created", async product => {
  ...
  const users = await User.findAll({
    where: {
      receive_notifications: true,
      id: {
        [_sequelize.Op.ne]: user.id // exclude the user who added the product
      }
    }
  });
...
eventEmitter.on("sale:ended", async sale => {
  ...
  const users = await User.findAll({
    where: {
      receive_sale_notifications: true
    }
  });
  if (!users.length) {
    return;
...
eventEmitter.on("product:removed", async product => {
  ...
  const emails = users.map(async user => {
    return _sendEmail.default.sendNotification(user.email, subject, text);
  });
  ...
  await _sendEmail.default.sendNotification(seller.email, sellersubject, selletext);
  console.log("Notification sent to seller:", seller.name);
});
eventEmitter.on("product:checkout", async order => {
  ...
  await Promise.all([_sendEmail.default.sendNotification(email, subject, text), _sendEmail.default.sendNotification(user.email, subject, text)]);
});
