...
class Cart {
  static cartItem = async cartData => {
    try {
      const Carts = await _index.default.Cart_items.findOne({
        where: {
      ...
      if (!Carts) {
        ...
        const CartsItem = await _index.default.Cart_items.create(cartData);
        return CartsItem;
      }
  ...
  static getCartItems = async user => {
    try {
      const Carts = await _index.default.Cart_items.findAll({
        where: {
          User_id: user
        },
        include: 'product'
